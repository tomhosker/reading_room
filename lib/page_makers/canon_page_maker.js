...
class BooksPageMaker extends CanonPageMaker {
    getTitle() {
        return "Books";
    }
    ...
    processHeading(heading) {
        ...
        if (heading === "inCatalogue") {
            return 'In <a href="/academy/canons/catalogue">Catalogue</a>';
        }
    ...
    processItem(item, key, record) {
        let result;

        if (item === null) return constants.NULL_MARKER;

        if (key === "title") {
            result = '<a href="' + record.link + '"><i>' + item + "</i></a>";
    ...
    makeCanonData(raw) {
        ...
        for (let property in raw[0]) {
            if (raw[0].hasOwnProperty(property)) {
                ...
                if (property !== "link") result.headings.push(heading);
            }
        }
        ...
        for (let record of raw) {
            ...
            for (let key of rawHeadings) {
                item = record[key];
                item = this.processItem(item, key, record);

                if (key !== "link") row.push(item);
            }
