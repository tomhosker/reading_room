...
class BooksRetriever extends CanonRetriever {
    getPageMakerClass() {
        return BooksPageMaker;
    }

    getMediaQuery() {
        const result =
            "SELECT Book.id, Book.title, Book.year_published, Book.link, " +
                "Book.in_catalogue, Book.genre, Book.notes, " +
                "Author.full_title " +
            "FROM Book " +
            "JOIN BookGenre ON BookGenre.code = Book.genre " +
            "LEFT JOIN Author ON Author.code = Book.author " +
            "ORDER BY BookGenre.seniority, Author.surname, " +
                "Author.full_title, Book.year_published, Book.title;";
        return result;
    }
}
