...
const SELECT_BLOODLINKS = "SELECT * FROM BloodLink WHERE senior = ? OR junior = ?;";
const SELECT_ACCOLADES = "SELECT * FROM Holds WHERE person = ?;";
...
class AccoldadeORM extends ORM {
    fetchCore() {
        const result = this.fetch(SELECT_ACCOLADE, [this.key]);
    ...
    fill() {
        this.data = this.fetchCore();
        this.data.chivalric = this.fetch(SELECT_CHIVALRIC, [this.data.chivalric]);
    }
}
